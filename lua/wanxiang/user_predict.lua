...
-- 量词兜底字符串
local default_classifiers   =
"百千万亿个多只名位口头匹条群批伙张把件台部块根颗粒滴片朵面扇顶栋座所辆艘架盏支枝杆双对副套打串束排阵堆叠摞扎杯瓶盒包份碗锅盆桶袋罐盘次场局回趟顿番遍声项宗桩款步招年月天周岁秒分刻代期届任夜季本册篇首句段卷幅节堂门帖字行米寸尺里斤两吨克升元角毛笔"
...
local function get_db(env)
  local config = env.engine.schema.config
  local db_name = config:get_string("user_predict/db_name") or "userdatabase/predict"
...
function F.func(input, env)
  ...
  local current_input = ctx.input or ""
  local do_fallback = CONFIG.ENABLE_FALLBACK_REORDER and current_input == shared_reverted_code and
      shared_reverted_code ~= ""
  ...
  if do_fallback then
    ...
    for cand in input:iter() do
      ...
      if idx == 1 then
        c1 = cand
      elseif idx == 2 then
        local is_cand_valid = cand.type ~= "raw" and cand.type ~= "english" and
            not s_find(cand.text or "", "^[a-zA-Z]+$")
