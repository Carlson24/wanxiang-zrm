...
local function expand_code_variant(main_projection, xlit_projection, part)
  ...
  local function add(s) if s and #s > 0 and not seen[s] then
      seen[s] = true
      table.insert(out, s)
    end end
  local function add_xlit(s) if s and #s > 0 and not seen_xlit[s] then
      seen_xlit[s] = true
                ...
                if ok and translation then
                  ...
                  for c in translation:iter() do
                    if get_utf8_len(c.text) == 2 and c.text ~= orig_phrase_text then
                      ...
                      -- 模式A：左变右不变 (例如: 星星 -> 行星)
                      local case_a = (char2 == orig_char2) and
                      check_char_fuma_match(env, pure_pinyin_parts[1], fuma, char1)
                      -- 模式B：左不变右变 (例如: 星星 -> 星形)
                      local case_b = (char1 == orig_char1) and
                      check_char_fuma_match(env, pure_pinyin_parts[2], fuma, char2)
